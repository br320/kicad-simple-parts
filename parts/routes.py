...
@router.put("/parts/{part_id}.json", response_model=Part)
async def update_part(part_id: int, update: PartUpdate, session: Session = Depends(get_session)) -> Part:
    return crud.update_part(part_id, update, session)
